...
- **Unreadable repository indexes were indistinguishable from repos that simply lack the chart.** A missing or corrupt `<repo>-index.yaml` was silently skipped, so a chart could be reported as available in one repo when a second repo was merely unreadable. Load failures are now collected per repo and reported to stderr under `--debug`.
